...
import { FileText, User, Activity, Stethoscope, ChevronRight, Mic } from "lucide-react";
...
export default function Session() {
  ...
  const handleTranscriptionReady = useCallback((transcription: string) => {
    setTranscription(transcription);
  }, []);

  // Refresh the transcription from DB when local transcription changes
  useEffect(() => {
    if (transcription && currentSessionId) {
      // Delay to ensure DB has been updated before refreshing
  ...
  // Memoize patient info props for PatientInfoCard
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      <Sidebar />
      <div className="flex-1 overflow-auto pl-16 lg:pl-64 scroll-smooth">
        <div className="container mx-auto py-8 px-4 md:px-8 max-w-[1600px]">
            ...
            {currentSessionId && (
              <div className="flex items-center gap-2 bg-muted/40 px-4 py-2 rounded-full border border-border/50 animate-in fade-in slide-in-from-right-5">
                 ...
                 <span className="text-sm font-medium text-muted-foreground">
                   Sesión Activa: <span className="font-mono text-foreground ml-1">{currentSessionId.substring(0, 8)}</span>
                 </span>
              </div>
            )}
          </header>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 h-full pb-10">
            {/* Left Column: Patient & Recording (4 columns on XL) */}
              ...
              <Card className="overflow-hidden border-none shadow-md bg-gradient-to-b from-card to-muted/20">
                <CardHeader className="pb-4 border-b border-border/50 bg-muted/30">
                  <CardTitle className="flex items-center justify-between text-lg font-semibold">
                    ...
                    {selectedPatient && (
                      <Badge variant="secondary" className="font-normal bg-background/80 backdrop-blur-sm">
                        Seleccionado
                      </Badge>
                    )}
                  </CardTitle>
                  ...
                  {selectedPatient ? (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                      <div className="flex items-center gap-4">
                        <div className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xl font-bold shadow-inner">
                          {selectedPatient.nombre.substring(0, 2).toUpperCase()}
                        </div>
                        <div className="flex-1 space-y-1">
                          <h3 className="font-bold text-xl text-foreground">{selectedPatient.nombre}</h3>
                          <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1">
                              <span className="font-medium text-foreground/80">DNI:</span> {selectedPatient.dni}
                            </span>
                            {selectedPatient.edad && (
                              <span className="flex items-center gap-1">
                                <span className="w-1 h-1 rounded-full bg-border mx-1" />
                                <span className="font-medium text-foreground/80">Edad:</span> {selectedPatient.edad} años
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                         <div className="p-3 rounded-lg bg-background border border-border/50 shadow-sm">
                            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mb-1">Diagnóstico</p>
                            <p className="text-sm font-medium truncate" title={selectedPatient.diagnostico || "Sin diagnóstico"}>
                              {selectedPatient.diagnostico || <span className="text-muted-foreground italic">Sin registro</span>}
                            </p>
                         </div>
                         <div className="p-3 rounded-lg bg-background border border-border/50 shadow-sm">
                            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mb-1">Procedencia</p>
                            <p className="text-sm font-medium truncate" title={selectedPatient.procedencia || "Sin procedencia"}>
                              {selectedPatient.procedencia || <span className="text-muted-foreground italic">Sin registro</span>}
                            </p>
                         </div>
                      </div>
                      ...
                      <Button 
                        ...
                        className="w-full text-muted-foreground hover:text-primary hover:bg-primary/5 border border-dashed border-border hover:border-primary/30"
                      >
                        Cambiar paciente
                      </Button>
                    </div>
            ...
            {/* Right Column: Medical Record (8 columns on XL) */}
                ...
                <CardContent className="flex-1 overflow-y-auto p-0 bg-muted/5">
                  {patientId && currentSessionId ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-center h-full px-6 py-20 text-muted-foreground animate-in fade-in zoom-in-95 duration-500">
                      <div className="h-24 w-24 rounded-full bg-muted/50 flex items-center justify-center mb-6 shadow-inner">
                         <FileText className="h-10 w-10 text-muted-foreground/40" />
                      </div>
                      <h3 className="text-xl font-semibold text-foreground mb-2">Ficha lista para completar</h3>
                      <p className="text-muted-foreground max-w-md mb-8 leading-relaxed">
                        Para comenzar a documentar la consulta, primero selecciona un paciente 
                        e inicia la grabación de la sesión.
                      </p>
                      
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-2xl text-sm">
                         <div className="flex flex-col items-center gap-2 p-4 rounded-lg border bg-background/50">
                            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">1</div>
                            <span>Selecciona Paciente</span>
                         </div>
                         <div className="flex items-center justify-center">
                            <ChevronRight className="h-6 w-6 text-muted-foreground/30 rotate-90 sm:rotate-0" />
                         </div>
                         <div className="flex flex-col items-center gap-2 p-4 rounded-lg border bg-background/50">
                            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">2</div>
                            <span>Graba Sesión</span>
                         </div>
                         <div className="flex items-center justify-center">
                            <ChevronRight className="h-6 w-6 text-muted-foreground/30 rotate-90 sm:rotate-0" />
                         </div>
