  ...
  const generateSessionId = async (): Promise<{ sessionId: string; dbSessionId: string } | null> => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('sesiones').insert({
        ...
        doctor_id: user.id,
      }).select('id, codigo_sesion, paciente_id');
      ...
      if (data && data.length > 0) {
        ...
        if (createdSession && createdSession.id) {
          ...
          return { sessionId: randomId, dbSessionId: createdSession.id };
        }
      }
      
      return null;
    } catch (error) {
      logger.error("Error in generateSessionId:", error);
      toast.error("Error al crear la sesión");
      setSessionId("");
      return null;
    }
    ...
    if (patientId) {
      try {
        ...
        if (dbSessionId) {
          ...
          logger.error("No session identifier available");
          return;
        }
        ...
        if (error) {
          logger.error("Error updating session with transcription:", error);
        ...
        logger.error("Error in updateSessionWithTranscription:", error);
      }
    }
  };
